...
class AgentsApiService extends ApiService {
  ...
  public async getAllAgents(): Promise<Agent[]> {
    return this.get<{ agents: Agent[] }>(this.endpoint).then(
  ...
  public async getAgent(id: string): Promise<Agent> {
    return this.get<{ agent: Agent }>(`${this.endpoint}/${id}`).then(
  ...
  public async createAgent(agent: AgentCreateParams): Promise<Agent> {
    return this.post<{ agent: Agent }, AgentCreateParams>(
      this.endpoint,
      agent
  ...
  public async updateAgent(
    ...
    return this.put<{ agent: Agent }, AgentUpdateParams>(
      `${this.endpoint}/${id}`,
      updates
    ).then((response) => response.agent);
  }
  ...
  public async runAgent(id: string): Promise<AgentRunResult> {
    return this.post<{ result: AgentRunResult }>(
  ...
  public async getAgentResults(id: string): Promise<AgentRunResult[]> {
    return this.get<{ results: AgentRunResult[] }>(
      `${this.endpoint}/${id}/results`
  ...
  public async duplicateAgent(id: string): Promise<Agent> {
    return this.post<{ agent: Agent }>(`${this.endpoint}/${id}/duplicate`).then(
      (response) => response.agent
    );
  }
}
