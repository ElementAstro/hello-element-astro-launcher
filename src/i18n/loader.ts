...
import { translationKeys as environmentTranslations } from "@/components/environment/translations";
 ...
 * @returns 页面翻译字典
 */
export function getPageTranslations(page: string, locale: string = defaultLocale) {
  const validLocale = supportedLocales.includes(locale) ? locale : defaultLocale;
  
  // 基础翻译
  const baseDict = { ...baseTranslations[validLocale as keyof typeof baseTranslations] };
  ...
  switch (page) {
    case 'proxy':
      return {
        ...baseDict,
        proxy: validLocale === 'zh-CN' ? proxyTranslationsZh : proxyTranslationsEn,
      };
    case 'home':
      return baseDict;
    case 'tools':
 ...
 * @returns 组件翻译字典
 */
export function getComponentTranslations(component: string, locale: string = defaultLocale) {
  const validLocale = supportedLocales.includes(locale) ? locale : defaultLocale;
  
  switch (component) {
    case 'proxy':
      return validLocale === 'zh-CN' ? proxyTranslationsZh : proxyTranslationsEn;
    case 'launcher':
      return launcherTranslations[validLocale as keyof typeof launcherTranslations];
    case 'home':
      return homeTranslations[validLocale as keyof typeof homeTranslations];
    case 'tools':
      return toolsTranslations[validLocale as keyof typeof toolsTranslations];
    case 'environment':
